...
func GetPCLQTemplateHashes(cliqueHashes map[string]string, pcs *grovecorev1alpha1.PodCliqueSet, pcsg *grovecorev1alpha1.PodCliqueScalingGroup) map[string]string {
	...
	for pcsgReplicaIndex := range int(pcsg.Spec.Replicas) {
		for _, pclqTemplateSpec := range pclqTemplateSpecs {
			...
			hash, err := GetSelectedPCLQPodTemplateHash(cliqueHashes, pclqTemplateSpec.Name)
			if err != nil {
				continue
			}
...
func GetPCLQsInPCSGPendingUpdate(cliqueHashes map[string]string, pcs *grovecorev1alpha1.PodCliqueSet, pcsg *grovecorev1alpha1.PodCliqueScalingGroup, existingPCLQs []grovecorev1alpha1.PodClique) []string {
	pclqFQNsPendingUpdate := make([]string, 0, len(existingPCLQs))
	expectedPCLQPodTemplateHashes := GetPCLQTemplateHashes(cliqueHashes, pcs, pcsg)
